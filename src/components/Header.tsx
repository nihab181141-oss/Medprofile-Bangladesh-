        ...
        <nav className="hidden xl:flex items-center gap-5 lg:gap-7 text-[13px] font-bold text-slate-600">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              to={link.path}
              className={`transition-colors relative py-1.5 after:absolute after:bottom-0 after:left-0 after:h-0.5 after:w-0 hover:after:w-full after:bg-teal-600 after:transition-all after:duration-300 flex items-center gap-1.5 ${
                isActive(link.path) ? "text-teal-600 font-extrabold after:w-full" : "hover:text-teal-600 text-slate-600"
              }`}
            >
              {link.name}
              ...
              {link.name === "Templates" && <span className="text-[8px] font-extrabold text-white bg-teal-600 px-1.5 py-0.5 rounded leading-none">6 LIVE</span>}
            </Link>
          ))}
        </nav>
            ...
            {navLinks.map((link) => (
              <Link 
                ...
                className={`w-full text-left py-2.5 px-4 text-sm font-bold rounded-lg transition-colors flex items-center justify-between ${
                  isActive(link.path) ? "bg-teal-50 text-teal-600 font-extrabold" : "text-slate-700 hover:bg-slate-50 hover:text-teal-600"
                }`}
              >
                <span>{link.name}</span>
                ...
                {link.name === "Templates" && <span className="text-[8px] uppercase tracking-wider font-extrabold text-teal-600 bg-teal-50 px-2 py-0.5 rounded border border-teal-200">6 Designs</span>}
              </Link>
            ))}
            <Link 
              to="/consultation"
              onClick={() => setMobileMenuOpen(false)}
              className="w-full text-left py-2.5 px-4 text-sm font-bold text-slate-700 hover:bg-slate-50 hover:text-teal-600 rounded-lg transition-colors flex items-center justify-between"
            >
