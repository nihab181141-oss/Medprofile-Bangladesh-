  ...
  return (
    <div className="pt-24 min-h-screen bg-slate-50/50">
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-xs font-bold transition-all duration-300 cursor-pointer ${
                    selectedCategory === cat
                      ? "bg-slate-900 text-brand-400 shadow-md scale-105"
            ...
            <div className="relative w-full md:w-80">
              ...
              <input
                ...
                className="w-full text-xs py-3 pl-10 pr-4 bg-slate-55 border border-slate-200 rounded-full focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-600 transition-all font-medium text-slate-800"
              />
              {searchQuery && (
                <button 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredTemplates.map((tpl, idx) => (
              <motion.div
                ...
                className="bg-slate-950 rounded-2xl border border-slate-800/70 overflow-hidden shadow-lg hover:shadow-[0_25px_50px_-12px_rgba(20,184,166,0.18)] hover:border-brand-500/50 hover:scale-105 flex flex-col justify-between group transition-all duration-500 ease-out"
              >
                ...
                <div className="bg-slate-900 border-b border-slate-800 px-4 py-3 flex items-center justify-between shrink-0 select-none">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    <span className="w-2.5 h-2.5 rounded-full bg-brand-500/90 shadow-sm shadow-brand-500/10"></span>
                    <span className="w-2.5 h-2.5 rounded-full bg-brand-500/90 shadow-sm shadow-brand-500/10"></span>
                    <span className="w-2.5 h-2.5 rounded-full bg-brand-500/90 shadow-sm shadow-brand-500/10"></span>
                  </div>
                ...
                <div className="relative aspect-[16/10] w-full overflow-hidden bg-slate-900 border-b border-slate-800 select-none">
                  ...
                  {!loadedFrames[tpl.id] && (
                    <div className="absolute inset-0 bg-slate-950 z-10 flex flex-col justify-between p-5 animate-pulse">
                      ...
                      <div className="space-y-2 my-auto text-center">
                        ...
                        <div className="h-5 w-16 bg-brand-500/10 border border-brand-500/20 rounded mx-auto mt-2"></div>
                      </div>
                  ...
                  <div className="absolute top-4 left-4 z-20 flex gap-2 pointer-events-none">
                    <span className="text-[9px] font-extrabold uppercase bg-slate-950/80 backdrop-blur-md border border-slate-800 text-brand-300 px-2.5 py-1 rounded-full shadow-md leading-none">
                      Template {tpl.id}
                    </span>
                    {tpl.isPopular && (
                      <span className="text-[9px] font-extrabold uppercase bg-brand-500 text-slate-950 px-2.5 py-1 rounded-full shadow-md flex items-center gap-1 leading-none">
                        🏆 Popular
                      </span>
                    )}
                  </div>
                  ...
                  <div className="absolute bottom-4 right-4 z-20 pointer-events-none">
                    <span className="inline-flex items-center gap-1.5 bg-brand-500/95 backdrop-blur-md text-slate-950 text-[9px] font-black px-2.5 py-1 rounded-full shadow-md uppercase tracking-wider leading-none">
                ...
                <div className="p-6 flex-1 flex flex-col justify-between space-y-5 bg-slate-950">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between text-[10px] font-bold tracking-wider uppercase leading-none">
                      <span className="text-brand-300 bg-brand-500/10 border border-brand-500/20 px-2.5 py-1 rounded-full">
                        {tpl.category} Mode
                      </span>
                    ...
                    <h3 className="font-display font-extrabold text-lg text-white group-hover:text-brand-400 transition-colors duration-300 leading-snug">
                      {tpl.name}
                    </h3>
                  ...
                  <div className="pt-1">
                    <a
                      ...
                      className="w-full inline-flex items-center justify-center gap-2 py-3 bg-brand-500 hover:bg-brand-400 text-slate-950 rounded-xl text-xs font-bold transition-all duration-300 hover:scale-105 shadow-md hover:shadow-brand-500/10 active:scale-95 group/btn cursor-pointer"
                    >
      ...
      <section className="bg-gradient-to-br from-slate-900 to-slate-950 border-t border-slate-800 text-white py-16 md:py-24 overflow-hidden relative">
        <div className="absolute top-1/2 -translate-y-1/2 left-0 w-80 h-80 bg-brand-500/10 rounded-full filter blur-[100px] pointer-events-none"></div>
        <div className="absolute top-1/2 -translate-y-1/2 right-0 w-80 h-80 bg-brand-500/10 rounded-full filter blur-[100px] pointer-events-none"></div>
        ...
        <div className="max-w-4xl mx-auto px-4 text-center space-y-8 relative z-10">
          ...
          <div className="pt-4 flex flex-col sm:flex-row justify-center items-center gap-4">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2.5 px-8 py-4 bg-brand-500 hover:bg-brand-400 text-slate-950 rounded-full text-sm font-extrabold tracking-wide shadow-lg shadow-brand-500/20 hover:-translate-y-0.5 transition-all duration-300"
            >
