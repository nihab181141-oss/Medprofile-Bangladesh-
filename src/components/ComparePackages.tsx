...
export default function ComparePackages() {
  ...
  const renderFeatureValue = (val: boolean | string) => {
    ...
    return <span className="text-sm font-semibold text-slate-200">{val}</span>;
  };
  ...
  return (
    <section id="compare-packages" className="py-24 bg-slate-950 relative overflow-hidden border-t border-slate-900">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[60rem] h-[60rem] bg-teal-500/5 rounded-full blur-[120px] pointer-events-none -z-10"></div>
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <p className="mt-4 text-slate-400 text-sm md:text-base leading-relaxed max-w-2xl mx-auto font-medium">
            Find the perfect solution for your healthcare business by comparing our packages side by side.
          </p>
        </motion.div>

        {/* Desktop Table */}
        <div id="compare-table-container" className="hidden lg:block relative bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-3xl overflow-hidden shadow-2xl mb-24">
          <table className="w-full text-left border-collapse">
            <thead className={`bg-slate-900/90 backdrop-blur-md z-20 ${isSticky ? 'sticky top-16 shadow-xl border-b border-slate-800' : 'border-b border-slate-800'}`}>
              <tr>
                <th className="w-1/4 p-6"></th>
                {packages.map(pkg => (
                  <th key={pkg.id} className={`w-[18.75%] p-6 text-center border-l border-slate-800/50 relative ${pkg.mostPopular ? 'bg-slate-800/50' : ''}`}>
                    {pkg.mostPopular && (
                      <div className="absolute top-0 left-0 right-0 bg-gradient-to-r from-teal-500 to-teal-400 text-slate-950 text-[9px] font-black uppercase tracking-widest py-1">
                        Most Popular
                      </div>
                    )}
                    <div className={`mt-2 flex flex-col items-center justify-center ${pkg.mostPopular ? 'mt-4' : ''}`}>
                      <div className="mb-3">{pkg.icon}</div>
                      <h3 className="font-display font-extrabold text-white text-lg">{pkg.name}</h3>
                    </div>
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {features.map((feature, idx) => (
                <tr key={idx} className="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors">
                  <td className="p-5 text-sm font-semibold text-slate-300 pl-8">{feature.name}</td>
                  <td className="p-5 text-center border-l border-slate-800/50">{renderFeatureValue(feature.doctor)}</td>
                  <td className="p-5 text-center border-l border-slate-800/50 bg-slate-800/10">{renderFeatureValue(feature.dental)}</td>
                  <td className="p-5 text-center border-l border-slate-800/50">{renderFeatureValue(feature.hospital)}</td>
                  <td className="p-5 text-center border-l border-slate-800/50">{renderFeatureValue(feature.enterprise)}</td>
                </tr>
              ))}
              {/* Best For Row */}
              <tr className="bg-slate-900/60 border-t-2 border-slate-700">
                <td className="p-8 align-top">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-display font-extrabold text-white text-lg tracking-wide">Best For</span>
                  </div>
                </td>
                {packages.map(pkg => (
                  <td key={`best-${pkg.id}`} className={`p-6 align-top border-l border-slate-800/50 ${pkg.mostPopular ? 'bg-slate-800/30' : ''}`}>
        ...
        <div className="lg:hidden space-y-12 mb-24">
          {packages.map(pkg => (
            <div key={pkg.id} className={`bg-slate-900/50 border rounded-3xl overflow-hidden ${pkg.mostPopular ? 'border-teal-500/50 shadow-[0_10px_30px_rgba(20,184,166,0.15)]' : 'border-slate-800 shadow-xl'}`}>
              {pkg.mostPopular && (
                <div className="bg-gradient-to-r from-teal-500 to-teal-400 text-slate-950 text-[10px] font-black uppercase tracking-widest text-center py-2">
                  Most Popular
                </div>
              )}
              ...
              <div className="p-6 space-y-4">
                {features.map((feature, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex items-center justify-between border-b border-slate-800/50 pb-3 last:border-0 last:pb-0">
                      <span className="text-xs font-semibold text-slate-300">{feature.name}</span>
                      <div className="text-right pl-4">
                        {val === true ? <Check className="w-4 h-4 text-emerald-400 ml-auto" /> : <span className="text-xs font-bold text-white">{val}</span>}
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="p-6 bg-slate-900 border-t border-slate-800">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="font-display font-bold text-white text-sm">Best For</span>
                </div>
        ...
        {/* CTA Section */}
        <motion.div 
          className="max-w-4xl mx-auto bg-gradient-to-br from-slate-900 to-slate-800 border border-slate-700/50 rounded-3xl p-10 md:p-14 text-center shadow-2xl relative overflow-hidden"
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 w-64 h-64 bg-sky-500/10 rounded-full blur-[80px] pointer-events-none -z-10"></div>
          
          <h3 className="font-display font-extrabold text-3xl md:text-4xl text-white tracking-tight mb-4">
            Still Not Sure Which Package Is Right For You?
          </h3>
          <p className="text-slate-400 text-sm md:text-base mb-10 max-w-2xl mx-auto font-medium leading-relaxed">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link 
              ...
              className="w-full sm:w-auto px-8 py-4 bg-teal-500 hover:bg-teal-400 text-slate-950 rounded-xl font-display font-bold text-xs uppercase tracking-widest transition-all duration-300 shadow-lg shadow-teal-500/20 active:scale-95 flex items-center justify-center gap-2"
            >
            ...
            <a 
              ...
              className="w-full sm:w-auto px-8 py-4 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-display font-bold text-xs uppercase tracking-widest transition-all duration-300 shadow-md border border-slate-700 active:scale-95 flex items-center justify-center gap-2"
            >
